...
class CompanyService {
  ...
  static async getBySlug(slug, projection = '') {
    const company = await Company.findOne({ slug }, projection).populate('logo').exec();
  ...
  static async create(data, options = {}) {
    try {
      ...
      company.settings.default_currency = i18next
        .t(`countries:countries.${company.country}.currencies`, {
          returnObjects: true,
        })
        .find(
          (currency) =>
            currency.label ===
            i18next.t(`countries:countries.${company.country}.default_currency`, {
              returnObjects: true,
            })
  ...
  static async getNextInvoiceSequenceValue(id) {
    const company = await Company.findOneAndUpdate(
      { _id: id },
      { $inc: { 'settings.current_invoice_sequence': 1 } },
      { new: true }
    );
  ...
  static async getNextCreditNoteSequenceValue(id) {
    const company = await Company.findOneAndUpdate(
      { _id: id },
      { $inc: { 'settings.current_credit_note_sequence': 1 } },
      { new: true }
    );
    return company.settings.current_credit_note_sequence;
  }

  static async getNextQuoteSequenceValue(id) {
    const company = await Company.findOneAndUpdate(
      { _id: id },
      { $inc: { 'settings.current_quote_sequence': 1 } },
      { new: true }
    );
    return company.settings.current_quote_sequence;
  }
  ...
  static async updateUsers(companyId, users) {
    try {
      ...
      console.log(
        'users.map(id => new ObjectId(id)):',
        users.map((id) => new ObjectId(id))
      );
      return await Company.findOneAndUpdate(
        { _id: companyId },
        { users: users.map((id) => new ObjectId(id)) },
        { new: true }
      );
    } catch (err) {
