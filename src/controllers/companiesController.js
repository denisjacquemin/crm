...
async function updateInvoiceSequence(req, res, next) {
  ...
  try {
    ...
    await CompanyService.updateById(req.session.current_company._id, {
      $set: { 'settings.current_invoice_sequence': value },
    });
    ...
    return res.status(200).json({
      message: req.i18n.t('settings.current_invoice_sequence_updated'),
    });
  } catch (err) {
  ...
  try {
    if (req.session.current_company.slug === req.params.slug) {
      return next({
        ...
        notification: {
          message: req.i18n.t('companies.controller.cannot_delete_current_company'),
          type: 'error',
        },
      });
    }
...
async function update(req, res, next) {
  try {
    ...
    if (req.body.value.settings) {
      if (
        req.body.value.settings.default_currency &&
        typeof req.body.value.settings.default_currency === 'string'
      ) {
        req.body.value.settings.default_currency = JSON.parse(
          req.body.value.settings.default_currency
        );
      }
    }
...
async function setDefaultInvoiceDueDateTermsType(req, res, next) {
  try {
    ...
    await CompanyService.update(req.session.current_company._id, {
      $set: {
        'settings.default_invoice_due_date_terms_type':
          req.body.default_invoice_due_date_terms_type,
      },
    });

    // Update the default_invoice_due_date_terms_type field on the user's session
    req.session.current_company.settings.default_invoice_due_date_terms_type =
      req.body.default_invoice_due_date_terms_type;

    // Send a success response
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.default_invoice_due_date_terms_type_updated'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function setSend_cc_to(req, res, next) {
  try {
    ...
    await CompanyService.update(req.session.current_company._id, {
      $set: { 'settings.send_cc_to': req.body.send_cc_to },
    });
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.send_cc_to_updated'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function setSend_bcc_to(req, res, next) {
  try {
    ...
    await CompanyService.update(req.session.current_company._id, {
      $set: { 'settings.send_bcc_to': req.body.send_bcc_to },
    });
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.send_bcc_to_updated'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function updateEmailTemplates(req, res, next) {
  try {
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.email_templates_updated'),
        type: 'success',
      },
    });
  } catch (err) {
  ...
  try {
    ...
    if (!taxrate || taxrate.value == null || !taxrate.label) {
      return res.status(400).json({
        notification: { message: 'Invalid tax rate data', type: 'error' },
      });
    }
    ...
    const existingTaxrateIndex = req.session.current_company.taxrates.findIndex(
      (tr) => tr._id.toString() === taxrate._id.toString()
    );
    if (existingTaxrateIndex !== -1) {
      req.session.current_company.taxrates[existingTaxrateIndex] = taxrate;
    } else {
      req.session.current_company.taxrates.push({
        ...taxrate,
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.taxrate_saved'),
        type: 'success',
      },
      taxrate: taxrate,
    });
...
async function deleteTaxRate(req, res, next) {
  try {
    ...
    if (!taxrate || !taxrate._id) {
      return res.status(400).json({
        notification: {
          message: req.i18n.t('companies.controller.invalid_taxrate_data'),
          type: 'error',
        },
      });
    }
    ...
    if (!updateResult) {
      return res.status(404).json({
        notification: {
          message: req.i18n.t('companies.controller.taxrate_not_found'),
          type: 'error',
        },
      });
    }
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.taxrate_deleted'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function showhidefields(req, res, next) {
  try {
    const documentType = req.query.document_type || 'invoice'; // Default to invoice if not specified
    const settingsPath = `settings.${documentType}`;

    // Update company settings for show/hide fields based on document type
    await CompanyService.update(req.session.current_company._id, {
      $set: {
        [`${settingsPath}.show_delivery_date`]: req.body.show_delivery_date === true,
        [`${settingsPath}.show_contact_person`]: req.body.show_contact_person === true,
        [`${settingsPath}.show_seller_email`]: req.body.show_seller_email === true,
        [`${settingsPath}.show_seller_website`]: req.body.show_seller_website === true,
        [`${settingsPath}.show_seller_phone`]: req.body.show_seller_phone === true,
        [`${settingsPath}.show_iban`]: req.body.show_iban === true,
        [`${settingsPath}.show_target_invoice`]: req.body.show_target_invoice === true,
      },
    });
    ...
    req.session.current_company.settings[documentType].show_delivery_date =
      req.body.show_delivery_date === true;
    req.session.current_company.settings[documentType].show_contact_person =
      req.body.show_contact_person === true;
    req.session.current_company.settings[documentType].show_seller_email =
      req.body.show_seller_email === true;
    req.session.current_company.settings[documentType].show_seller_website =
      req.body.show_seller_website === true;
    req.session.current_company.settings[documentType].show_seller_phone =
      req.body.show_seller_phone === true;
    req.session.current_company.settings[documentType].show_iban = req.body.show_iban === true;
    req.session.current_company.settings[documentType].show_target_invoice =
      req.body.show_target_invoice === true;
    // Send a success response
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.show_hide_fields_updated'),
        type: 'success',
      },
    });
  } catch (err) {
    console.error(`Error in companiesController.showhidefields `, err.message);
    next(err);
  }
}
...
async function showtargetinvoice(req, res, next) {
  try {
    ...
    await CompanyService.update(req.session.current_company._id, {
      $set: { 'settings.show_target_invoice': req.body.show_target_invoice },
    });

    // Update the show_target_invoice field on the user's session
    req.session.current_company.settings.show_target_invoice =
      req.body.show_target_invoice === 'true';

    // Send a success response
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.show_target_invoice_updated'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function defaultnotesoncreditnotes(req, res, next) {
  try {
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.default_notes_on_credit_notes_updated'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function defaulttaxrate(req, res, next) {
  try {
    ...
    await CompanyService.update(req.session.current_company._id, {
      $set: { default_taxrate: defaultTaxrateObjectId },
    });
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.default_vat_rate_updated'),
        type: 'success',
      },
    });
  } catch (err) {
...
async function updateCreditNoteNumbering(req, res, next) {
  try {
    const { credit_note_prefix, current_sequence } = req.body;
    ...
    return res.status(200).json({
      notification: {
        message: req.i18n.t('companies.controller.credit_note_numbering_updated'),
        type: 'success',
      },
    });
  } catch (err) {
    console.error(`Error in companiesController.updateCreditNoteNumbering `, err.message);
    next(err);
  }
}

async function updateInvoiceNumbering(req, res, next) {
  try {
    const { invoice_prefix, current_sequence } = req.body;

    // Only validate sequence, allow empty prefix
    if (!current_sequence || current_sequence < 0) {
      return res.status(400).json({
        notification: {
          message: req.i18n.t('companies.controller.sequence_must_be_positive'),
    ...
    await CompanyService.update(req.session.current_company._id, {
      $set: {
        'settings.invoice.prefix': invoice_prefix || '', // Store empty string if prefix is empty/null
        'settings.current_invoice_sequence': parseInt(current_sequence),
        'settings.invoice.reset_yearly': req.body.reset_yearly,
        'settings.invoice.sequence_increment': parseInt(req.body.sequence_increment),
      },
    });

    // Update session
    req.session.current_company.settings.invoice.prefix = invoice_prefix || '';
    req.session.current_company.settings.current_invoice_sequence = parseInt(current_sequence);
    req.session.current_company.settings.invoice.reset_yearly = req.body.reset_yearly;
    req.session.current_company.settings.invoice.sequence_increment = parseInt(
      req.body.sequence_increment
    );

    res.json({
      notification: {
        message: req.i18n.t('companies.controller.invoice_numbering_updated'),
        type: 'success',
      },
    });
  } catch (err) {
    console.error('Error updating invoice numbering:', err);
    res.status(500).json({
      notification: {
